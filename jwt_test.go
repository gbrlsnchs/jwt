...
func testJWT(t *testing.T, testCases []testCase) {
	for i, tc := range testCases {
		...
		t.Run(name, func(t *testing.T) {
			...
			aud := []string{fmt.Sprintf("test %d", i), "standard"}
			...
			if want, got := aud, jot2.GetAudience(); !reflect.DeepEqual(want, got) {
				t.Errorf("want %s, got %s", want, got)
			}
...
func testJWTSingleAud(t *testing.T, testCases []testCase) {
	for i, tc := range testCases {
		name := fmt.Sprintf("%s %s", tc.signer.String(), tc.verifier.String())
		t.Run(name, func(t *testing.T) {
			now := time.Now()
			...
			audResult := []string{fmt.Sprintf("test %d", i)}
			sub := fmt.Sprintf("sub %d", i)
			jti := strconv.Itoa(i)
			randomInt := rand.Intn(math.MaxUint32)
			randomFloat := rand.Float64() * 100
			...
			if want, got := audResult, jot2.GetAudience(); !reflect.DeepEqual(want, got) {
				t.Errorf("want %s, got %s", want, got)
			}
